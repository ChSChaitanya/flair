...
class ChainCRF(nn.Module):
    ...
    def decode(self, input, mask=None, leading_symbolic=0):
        ...
        pi[0] = energy[:, 0, -1, leading_symbolic:-1].view(pi[0].shape)
        pointer[0] = -1
        for t in range(1, length):
            pi_prev = pi[t - 1]
            tmp, pointer[t] = torch.max((energy_transpose[t] + pi_prev), dim=1)
            pi[t] = tmp.view(pi[t].shape)

        _, tmp = torch.max(pi[-1], dim=1)
        back_pointer[-1] = tmp.view(back_pointer[-1].shape)
